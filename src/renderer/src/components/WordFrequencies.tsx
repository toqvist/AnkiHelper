...
export default function WordFrequencies({
  getSentences,
  selectedDeck
}: WordFrequenciesProps): JSX.Element {
  ...
  useEffect(() => {
    if (wordFreqs.length == 0) getWordFreqs()
  }, [])
    ...
    <>
      ...
      {isLoading == false && (
        <div className="grid grid-cols-2">
          <button className="rounded-none" onClick={() => sortAlphabetically()}>
            A-Z
            {sortedBy === Sort.alphabeticallyAscending
              ? ' ▲'
              : sortedBy === Sort.alphabeticallyDescending
              ? ' ▼'
              : ''}
          </button>
          <button className="rounded-none" onClick={() => sortByFrequency()}>
            #
            {sortedBy === Sort.wordFrequencyAscending
              ? ' ▲'
              : sortedBy === Sort.wordFrequencyDescending
              ? ' ▼'
              : ''}
          </button>
          {wordFreqs.map((wordFreq, i) => {
            return (
              <>
                <div>
                  <a href="#" onClick={() => getSentences(wordFreq.word)}>
                    {wordFreq.word}
                  </a>
                </div>
