...
export default function ClozeModal({
  sentence,
  deck,
  closeModal,
  targetLanguage
}: ClozeModalProps): JSX.Element {
  ...
  async function getHints(sentence: Sentence, targetLanguage: string): Promise<Word[]> {
    ...
    const wordObjects: Word[] = await Promise.all(
  ...
  function normalizeAsSentence(): void {
    ...
    const normalizedSentence: Word[] = [...wordObjects]
            ...
            <div className="modal-column">
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm text-slate-950">
                  Words marked blue will be <span className="text-clozed">[clozed]</span>
                </p>
              </div>
                  ...
                  if (!wordObject.isPunctuation) {
                    return (
                      <ClozeWord
                        ...
                        translations={wordObject.translations}
                      />
                    )
                  } else {
                    return (
                      <span key={i} className="text-slate-950 text-3xl">
                        {wordObject.text}
                      </span>
                    )
                  }
              ...
              <div className="flex gap-1">
                <div className="relative">
                  <button
                    className="bg-slate-200 border-slate-200 border-2 text-slate-950 px-2 py-2 rounded-md text-sm hover:bg-slate-300 hover:border-slate-300"
                  ...
                  <div
                    className={`absolute z-10 right-0 mt-2 py-2 w-32 bg-slate-200 border-slate-300 rounded-md shadow ${
                ...
                <button
                  className="text-slate-950 bg-slate-200 border-2 rounded-md text-sm hover:bg-slate-300 hover:border-slate-300"
                  onClick={() => createCard()}
                  disabled={!anyWordIsClozed()}
                >
