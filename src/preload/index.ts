...
const api = {
  ...
  previewFile: (filePath: string, lines: number) =>
    ipcRenderer.invoke('file:preview', filePath, lines),
  readFileAsSentences: (filePath: string, lines: number) =>
    ipcRenderer.invoke('file:readFileAsSentences', filePath, lines),
  openSaveDialog: () => ipcRenderer.invoke('file:saveDialog'),
  saveFileTemp: (filePath: string, content: string) =>
    ipcRenderer.invoke('file:saveTemp', filePath, content),
  ...
  usedInSentences: (filePath: string, argWord: string) =>
    ipcRenderer.invoke('tools:usedInSentences', filePath, argWord),
  translate: (text: string, targetLanguage: string) =>
    ipcRenderer.invoke('tools:translate', text, targetLanguage)
}
